...
(function($) {
    ...
    $( document ).ready( function() {
    	...
    	$( 'input.has-dependents' ).each( function() {
    		you_can_quote_me_on_that_toggle_depdents.call( $( this ) );
    	});
    	
    	$( 'input.has-dependents' ).on('change', function(e){
    		you_can_quote_me_on_that_toggle_depdents.call( e.target );
    	});
    	
        // Initialize the color picker
    	$('.colorpicker').each( function() {
    		you_can_quote_me_on_that_init_color_picker.call( $( this ) );
    	});
    	
    	$('#you_can_quote_me_on_that_slide_overlay_opacity').on( 'change', function() {
    	});
        ...
        $( '.otb-form-control-dropdown_pages_posts' ).each( function() {
        	you_can_quote_me_on_that_toggle_slider_link_custom_field.call( $(this) );
        });
	    ...
	    function you_can_quote_me_on_that_toggle_slider_link_custom_field() {
	    	if ( $(this).val() == 'custom' ) {
		...
		$( '.delete-button' ).on( 'click', function(e) {
			...
			button_fields.first().slideUp(200, function() {
				...
				you_can_quote_me_on_that_toggle_slider_link_custom_field.call( button_fields.find( '.otb-form-control-dropdown_pages_posts' ) );
			} );
		...
		function you_can_quote_me_on_that_init_color_picker() {
			$(this)[0].jscolor = new jscolor($(this)[0], {
				'hash': true
			});
    	}

		// Show / hide the dependent fields
		function you_can_quote_me_on_that_toggle_depdents() {
			
			switch ( $( this ).data('fieldId') ) {
				case 'you_can_quote_me_on_that_display_directional_buttons':
					if ( $( this ).prop('checked') ) {
						$( '.you_can_quote_me_on_that_display_directional_button_settings' ).slideDown({
							duration: 200,
							easing: 'linear' 
						});
					} else {
						$( '.you_can_quote_me_on_that_display_directional_button_settings' ).slideUp({
							duration: 200,
							easing: 'linear' 
						});
					}
				...
				case 'you_can_quote_me_on_that_has_min_width':
					if ( $( this ).prop('checked') ) {
						$( '.you_can_quote_me_on_that_min_width_settings' ).slideDown({
							duration: 200,
							easing: 'linear' 
						});
					} else {
						$( '.you_can_quote_me_on_that_min_width_settings' ).slideUp({
							duration: 200,
							easing: 'linear' 
						});
					}
				...
				case 'you_can_quote_me_on_that_slideshow':
					if ( $( this ).is( ':checked' ) ) {
					//if ( $( this ).prop('checked') ) {
						$( '.you_can_quote_me_on_that_slideshow_settings' ).slideDown({
							duration: 200,
							easing: 'linear' 
						});
					} else {
						$( '.you_can_quote_me_on_that_slideshow_settings' ).slideUp({
							duration: 200,
							easing: 'linear' 
						});
					}
